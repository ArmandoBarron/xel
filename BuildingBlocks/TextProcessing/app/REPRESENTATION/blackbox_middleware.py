...
NAME_APPLICATION = "REPRESENTATION"
...
def execute(params):
    ...
    """ 
    inputzip = params['inputpath']+params['inputfile']+".zip"  #where the input data is
    #grobid_outputzip= params['inputpath']+params['outputfile']+"."+OUTPUT_DATA_FORMAT #where the input data is

    inputpath = params['inputpath']+params['inputfile']+"_folder"  #where the input data is
    #grobid_outputpath= params['inputpath']+params['outputfile']+"_folder" #where the input data is
    
    try:
        if not os.path.exists(inputpath): #create input folder
    ...
    with ZipFile(inputzip, 'r') as zipObj:
        ...
        zipObj.extractall(inputpath)
    
        
    params['input_distance_filename'] = inputpath +"/" +params['input_distance_filename']
    params['clustered_dataframe_filename'] = inputpath +"/" +params['clustered_dataframe_filename']
    params['outputfile']  = params['inputpath']+params['outputfile']+".png"
    LOGER.error(params)
