...
def execute(params):
    ...
    eval_path = ACTUAL_PATH

    eval_inputzip = params['inputpath']+params['inputfile']+"."+OUTPUT_DATA_FORMAT  #where the input data is (without the extension)
    eval_outputzip= params['inputpath']+params['outputfile']+"."+OUTPUT_DATA_FORMAT #where the output data must be

    # unzip folder
    with ZipFile(eval_inputzip, 'r') as zipObj:
        # Extract all the contents of zip file in different directory
        zipObj.extractall(eval_path)

    # call the application
    execution_status = os.system('python %sevaluate-ila.py --vocab_file %s --vectors_file %s > %seval.txt' %(eval_path, eval_path+params['vocabfile_name'], eval_path+params['vectorsfile_name'], eval_path ))

    # create a ZipFile object
    with ZipFile(eval_outputzip, 'w') as zipObj:
        zipObj.write(eval_path+"eval.txt", "eval.txt")
