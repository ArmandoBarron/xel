...
function demoflowy_showModalFromId(canvasId,canvasType) {
    ...
    html_2_add = ``
    //if (modal2open== "modal_edition")
    //{
    //    html_2_add = `
    //            <div class="form-check" style="text-align: right;">
    //                <button onclick="Show_configurations('${canvasId}')" type="button" class="btn btn-secondary"><i class="fas fa-cog"></i></button>
    //            </div>
    //    `
    //}
...
function Metadata_collector(){
    ...
    //obj_metadata.root_datasource_metadata = ROOT_METADATA //metadata from the first dataset
...
function Metadata_assign(meta,ignore_context=false){
    ...
    //ROOT_METADATA = meta.root_datasource_metadata //metadata from the first dataset
...
function Set_DatasourceMap(datasource_Obj,type_dataset)
{
    ...
    xel_describe_dataset(filename=datasource_Obj['filename']).done(function(result){
        ROOT_METADATA = JSON.parse(result).info
        notificarUsuario("Dataset's metadata obtained","success")
    });    
}
