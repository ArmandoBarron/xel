...
if(!empty($_SESSION['id'])){
  ...
  $number = $db->executeQuery($sql,array("iduser" => $iduser));
  $total_pages = ceil($number[0][0]/$per_page);
  ...
  foreach ($busquedas as $key => $busqueda) {
    //print_r($busqueda);
    $url = $server."/AEM-Eris/index.php?busqueda=".$busqueda['hash_key'];
    $url2 = "index.php?busqueda=".$busqueda['hash_key'];
    $ir = "<td><a href='$url2'><span class='glyphicon glyphicon-share-alt' aria-hidden='true'></span></a></td>";
