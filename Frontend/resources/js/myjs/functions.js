...
$(document).ready(function() {
    ...
    $("#hist_periodo").change(function(){
        $("#div_opt_periodo_historico_month").hide()
        $("#div_opt_periodo_historico_season").hide()

        var temporal = $("#hist_periodo").val();
        if (temporal=="season"){
            $("#div_opt_periodo_historico_season").show()
        }
        if (temporal=="month"){
            $("#div_opt_periodo_historico_month").show()
        }
    })
    ...
    $("#slShape2").change(function() {
        ...
        var espacialsl = $("#slShape2").val();
        var temporal = $("#slproductos").val();


        console.log(espacialsl)
            /*if (espacialsl == "estados" && temporal == "historico") {
                $('#slDiv').show();
                $('#slDiv2').show();
                $("#divDates").hide();
                $("#options1").hide();
        ...
        if ((espacialsl == "regiones" && temporal == "actual") || (espacialsl == "poligono" && temporal == "actual") ||
            (espacialsl == "estados" && temporal == "actual") || (espacialsl == "municipios" && temporal == "actual") ||
            (espacialsl == "ecoregion" && temporal == "actual")) {
            $('#slDiv').hide();
            $('#slDiv2').show();
            $("#divDates").show();
            $("#options1").hide();
        ...
        if ((espacialsl == "poligono" && temporal == "historico") || (espacialsl == "regiones" && temporal == "historico") ||
            (espacialsl == "estados" && temporal == "historico") || (espacialsl == "municipios" && temporal == "historico") ||
            (espacialsl == "ecoregion" && temporal == "historico")) {
            $('#slDiv').show();
            $('#slDiv2').show();
            $("#divDates").hide();
            $("#options1").hide();
            ...
            $("#regionDiv").hide();
            $("#ecoDiv").hide();
            notificarUsuario("Esta opcion no esta disponible por el momento", "danger");
    ...
    //TRAER INFORMACION DE LOS SHAPES DE REGIONES
    function get_shape_regions() {

        var id_regiones = "higroRegiones";
        $("#regionDiv").html("<h3> " + document.lngarr['regiones'] + "</h3>");
        $("#regionDiv").append("<select id=" + id_regiones + " onchange=\"draw_region()\" data-live-search=\"true\">");
        $("#higroRegiones").append("<option disabled selected> " + document.lngarr["select_region"] + "</option>");


        url = 'includes/get_region.php?'

        $.getJSON(url, function(data) {

            for (var i = 0; i < data.length; i++) {
                $("#higroRegiones").append("<option id=" + data[i]['id'] + " value= " + data[i]['id'] + "> " + data[i]['nombre'] + "</option>");
            }
            $("#regionDiv").append("</select>");
            $("#higroRegiones").addClass("form-control");
            $("#higroRegiones").addClass("selectpicker");
            $(".selectpicker").selectpicker('refresh');
        });
    }

    function get_shape_ecoregion() {
        var id_eco = "ecoregion";
        $("#ecoDiv").html("<h3> " + document.lngarr['eco'] + "</h3>");
        $("#ecoDiv").append("<select id=" + id_eco + " onchange=\"draw_ecoregion()\" data-live-search=\"true\">");
        $("#ecoregion").append("<option disabled selected> " + document.lngarr["select_eco"] + "</option>");

        url = 'includes/get_shape_eco.php?'

        $.getJSON(url, function(data) {

            for (var i = 0; i < data.length; i++) {
                $("#ecoregion").append("<option id=" + data[i]['id'] + " value= " + data[i]['id'] + "> " + data[i]['nombre'] + "</option>");
            }
            $("#ecooDiv").append("</select>");
            $("#ecoregion").addClass("form-control");
            $("#ecoregion").addClass("selectpicker");
            $(".selectpicker").selectpicker('refresh');
        });

    }

    function get_shape_states() {
        var id_states = "states";
        $("#statesDiv").html("<h3> " + document.lngarr['states'] + "</h3>");
        $("#statesDiv").append("<select id=" + id_states + " onchange=\"draw_state()\" data-live-search=\"true\">");
        $("#states").append("<option disabled selected> " + document.lngarr["select_region"] + "</option>");


        url = 'includes/get_shape.php?'

        $.getJSON(url, function(data) {

            for (var i = 0; i < data.length; i++) {
                // console.log(data[i]['nombre']);
                $("#states").append("<option id=" + data[i]['id'] + " value= " + data[i]['id'] + "> " + data[i]['nombre'] + "</option>");
            }
            $("#statesDiv").append("</select>");
            $("#states").addClass("form-control");
            $("#states").addClass("selectpicker");
            $(".selectpicker").selectpicker('refresh');
        });
    }

    function get_shape_mun() {
        console.log("entro a shape mun");
        //Se crea el select de estados sin ejecucion a dibujado
        var id_states = "states";
        $("#statesDiv").html("<h3> " + document.lngarr['states'] + "</h3>");
        $("#statesDiv").append("<select id=" + id_states + " onchange=\"draw_mun()\" data-live-search=\"true\">");
        $("#states").append("<option disabled selected> " + document.lngarr["select_region"] + "</option>");
    ...
    $('#toSatelitsTab').click(function(e) {
        e.preventDefault();
        $('#mytabs a[href="#options-tab"]').tab('show');
        numFuentes();
    });
    ...
    $('#backToOptions').click(function(e) {
        e.preventDefault();
        $('#mytabs a[href="#home-tab"]').tab('show');
    });
    ...
    $('#slproductos').on("change",function(){
        console.log("working")
        numFuentes()
    });
    ...
    $("#addAll").click(function(event) {
        ...
        if (selection == "actual") {
            ...
            console.log(Object.keys(document.diferenciales));
            tempdata = document.diferenciales;

            Object.keys(tempdata).forEach(key => {
                place = tempdata[key];
                place.forEach(function(item2, index2) {
                    DataToSave.push(item2);
                    temp = item2;
                });
            });

            CSVheaders = Object.keys(temp);
            console.log(temp);

            exportCSVFile(CSVheaders, DataToSave, "Diferencial_EMAS-MERRA");
            //addAllSearch();
        } else if (selection == "historico") {
            event.preventDefault();
            notificarUsuario("downloading Clustering data", 'info');

            liga=  "includes/get_file_historico.php?file="+document.diferenciales.clustfile;
            
            $("#ForDataDownload").html(`<a id='temporal_link' href='${liga}' target='_blank' ></a>`)
            $('#temporal_link').get(0).click();
            $("#ForDataDownload").html("")
            notificarUsuario("downloading RAW data", 'info');
            liga=  "includes/get_file_historico.php?file="+document.diferenciales.rawfile;
            $("#ForDataDownload").html(`<a id='temporal_link' href='${liga}' target='_blank' ></a>`)
            $('#temporal_link').get(0).click();
            $("#ForDataDownload").html("")

            //CSVheaders = ["Station", "latitud", "longitud", "year", "MAX EMAS", "MIN EMAS", "MAX MERRA", "MIN MERRA", "MAX DIFF", "MIN DIFF", "Etiqueta_clase"];
            //exportCSVFile(CSVheaders, document.historico, "historico_EMAS-MERRA");
        }
